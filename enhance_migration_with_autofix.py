...
# Find the migrate_doctypes function and enhance it
old_migrate_doctypes = '''def migrate_doctypes(origin_app, target_app, module, doctypes):
...
new_migrate_doctypes = '''def migrate_doctypes(origin_app, target_app, module, doctypes):
    """Migrate specific doctypes from origin to target app with auto-fix"""
    copied = 0
    errors = 0
    fixes_applied = 0
    ...
    for doctype in doctypes:
        try:
            ...
            if os.path.exists(source_base):
                ...
                # ✅ ENHANCED: Use comprehensive auto-fix function
                target_app_path = f"/home/frappe/frappe-bench/apps/{target_app}/{target_app}"
                fixed, message = auto_fix_doctype_naming(target_app_path, module, doctype)
                
                if fixed:
                    click.echo(f"      🔧 {message}")
                    fixes_applied += 1
                else:
                    # Fallback to original fix method for compatibility
                    target_json = f"{target_base}/{doctype}.json"
                    if os.path.exists(target_json):
                        with open(target_json, 'r') as f:
                            doctype_data = json.load(f)
...
content = content.replace(old_migrate_doctypes, new_migrate_doctypes)

# Write back
with open('app_migrator/commands/__init__.py', 'w') as f:
    f.write(content)

print("✅ Enhanced migrate_doctypes with comprehensive auto-fix")
