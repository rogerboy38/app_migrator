...
def setup_wizard(ctx, api_key, ssh_key, auto_approve, sync, test):
    ...
    if user_info:
        ...
        click.echo("⚠️  API key validation failed or not provided")
        if auto_approve or click.confirm("Continue as Guest? (limited functionality)", default=True):
            user_info = {"role": "guest", "email": "Guest User"}
            user_role = "guest"
            click.echo("👤 Continuing as Guest user")
        else:
...
def prompt_api_key(test_mode=False):
    ...
    if test_mode:
        click.echo("🧪 TEST MODE: Using test API key")
        return "fc_test_key_12345"
