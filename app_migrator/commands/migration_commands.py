...
def scan_site_impl(site_name: str, apps_filter: list[str] | None = None) -> SiteScanResult:
    """
    ...
    try:
        ...
        result.errors.append(f"Scan error: {e!s}")
...
def detect_conflicts(context, site, apps, output, output_format):
    ...
    try:
        ...
        if summary['total_conflicts'] > 0:
            ...
            click.echo("   Run 'generate-plan' with --config to create migration plan.")
        else:
            click.echo("\n✅ No conflicts detected! Apps are compatible for migration.")

    except Exception as e:
        click.echo(f"❌ Conflict detection failed: {e!s}")
        sys.exit(1)
...
def generate_plan(context, site, apps, target, config_file, output, output_format):
    ...
    """
    click.echo("📋 Generating migration plan")
    ...
    try:
        ...
        click.echo("   2. Adjust mappings and rules as needed")
        click.echo(f"   3. Run: bench app-migrator execute-plan --site {site} --plan {output} --dry-run")

    except Exception as e:
        click.echo(f"❌ Plan generation failed: {e!s}")
        sys.exit(1)
    ...
    try:
        ...
        if result.success:
            if dry_run:
                ...
                click.echo("   Run with --apply to execute changes")
            else:
        ...
        click.echo(f"❌ Execution failed: {e!s}")
        sys.exit(1)
