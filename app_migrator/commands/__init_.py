...
class InputValidator:
    """🛡️ Centralized input validation with best practices"""
    ...
    def validate_number_input(input_str, min_val=1, max_val=None, allow_back=True, allow_exit=True, allow_all=False, default_all=False):
        """
        🛡️ Centralized number input validation
        ...
        if not input_str:
            return False, None, "⚠️  Please enter a value"
        
        # Convert to number
        try:
            number = int(input_str)
        except ValueError:
            return False, None, f"❌ Invalid input '{input_str}'. Please enter a number."
        
        # Validate range
        if max_val and (number < min_val or number > max_val):
            return False, None, f"❌ Invalid choice. Please enter {min_val}-{max_val}"
        elif number < min_val:
            return False, None, f"❌ Number too small. Minimum is {min_val}"
    ...
    def validate_choice_input(input_str, choices, allow_back=True, allow_exit=True, allow_all=False, default_all=False):
        """
        🛡️ Centralized choice input validation
    ...
    def get_validated_input(prompt, validation_func, **kwargs):
        """
        🛡️ Get input with validation and retry logic
        """
        while True:
            try:
    ...
    def format_menu_options(choices, allow_back=True, allow_exit=True, allow_all=False, default_all=False):
        """
        📋 Format menu with standard options
        """
        options = []
        ...
        if allow_back:
            options.append(f"   {InputValidator.BACK_KEY}. ↩️ Back")
        if allow_all:
            if default_all:
                options.append(f"   {InputValidator.DEFAULT_ALL}. ✅ All (default - press Enter)")
            else:
                options.append(f"   {InputValidator.DEFAULT_ALL}. ✅ All")
        if allow_exit:
            options.append(f"   {InputValidator.EXIT_KEY}. 🚪 Exit")
...
class MultiSiteSession:
    ...
    def connect_to_site(self, site_name):
        ...
        try:
            if site_name not in self.available_sites:
                print(f"❌ Site '{site_name}' not found. Available sites: {self.available_sites}")
                return False
            ...
            print(f"❌ Failed to connect to site '{site_name}': {e}")
            return False
    ...
    def ensure_connection(self, site_name=None):
        ...
        if not target_site:
            print("❌ No site specified and no current connection")
            return False
...
class EnterpriseMigrationEngine:
    """
    🏗️ MULTI-SITE ENTERPRISE MIGRATION ENGINE
    ...
    def interactive_site_selection(self, step_name="SOURCE SITE"):
        ...
        print(f"   {validator.BACK_KEY}. ↩️ Back")
        print(f"   {validator.EXIT_KEY}. 🚪 Exit")
        ...
        while True:
            ...
            if result == 'EXIT':
                ...
                print("   ↩️ Returning to previous step...")
                return 'BACK'
    ...
    def select_target_site(self, source_site):
        ...
        if not sites:
            print("❌ No other sites available for target")
            return None
    ...
    def configure_namespace_strategy(self, source_app, target_app):
        """
        🎯 STEP 31-2: Automatic Renaming with Namespace Prefixes
        ...
        print(f"🤖 Auto-generated namespace prefix: '{auto_prefix}'")
        ...
        if custom_choice == 'y':
            ...
            if custom_prefix:
                ...
                print(f"✅ Using custom prefix: '{self.namespace_prefix}'")
            else:
                self.namespace_prefix = auto_prefix
                print(f"✅ Using auto-generated prefix: '{self.namespace_prefix}'")
        else:
            self.namespace_prefix = auto_prefix
            print(f"✅ Using auto-generated prefix: '{self.namespace_prefix}'")
    ...
    def migrate_cross_site_enterprise(self, source_site, target_site, source_app, target_app, modules=None):
        """
        🏗️ GOLD PRIZE: Complete Cross-Site Enterprise Migration
        """
        print(f"\n🏗️ ENTERPRISE CROSS-SITE MIGRATION")
        print("=" * 50)
        ...
        print(f"\n📊 STEP 1: Analyzing source site '{source_site}'...")
        source_modules = self._get_app_modules(source_app)
        if not source_modules:
            print("❌ No modules found in source app")
            return False
        ...
        print(f"\n📊 STEP 2: Analyzing target site '{target_site}'...")
        ...
        if conflicts:
            print(f"⚠️  Namespace conflicts detected: {list(conflicts)}")
            ...
            for conflict in conflicts:
                ...
                print(f"   🔄 Resolving conflict: {conflict} → {secondary_name}")
        
        # Step 5: Execute migration
        print(f"\n🚀 STEP 3: Executing cross-site migration...")
    ...
    def _get_app_modules(self, app_name):
        ...
        try:
            ...
            print(f"❌ Error getting modules for {app_name}: {e}")
            return []
    ...
    def _execute_cross_site_migration(self, source_site, target_site, source_app, target_app, source_modules, namespace_modules):
        ...
        try:
            ...
            print(f"📦 Migrating {len(source_modules)} modules:")
            
            for i, (source_module, new_name) in enumerate(zip(source_modules, namespace_modules), 1):
                print(f"   {i}. {source_module['module_name']} → {new_name}")
            ...
            if source_path.exists():
                print(f"   ✅ Source app path: {source_path}")
            if target_path.exists():
                print(f"   ✅ Target app path: {target_path}")
            
            # Final confirmation
            confirm = input(f"\n🚀 Proceed with enterprise migration? (y/N): ").strip().lower()
            if confirm != 'y':
                print("🚫 Migration cancelled")
                return False
            
            print("🎉 ENTERPRISE MIGRATION INITIATED!")
            print("   (Actual data transfer would happen here)")
            
            return True
            
        except Exception as e:
            print(f"❌ Cross-site migration failed: {e}")
            import traceback
            traceback.print_exc()
            return False
...
def interactive_enterprise_wizard():
    """
    🚀 ENTERPRISE MULTI-SITE MIGRATION WIZARD v3.1
    Fixed site-specific app filtering and input validation
    """
    print("🚀 ENTERPRISE MULTI-SITE MIGRATION WIZARD v3.1")
    print("=" * 55)
    ...
    strategies = [
        "🛡️ SAFE CUSTOMIZATION (Recommended)",
        "🚨 COMPLETE SACRIFICE (Advanced)"
    ]
    ...
    while True:
        ...
        if result == 'EXIT':
            ...
            if strategy_choice == 2:
                confirm = input("   🚨 TYPE 'SACRIFICE' to confirm source app will be destroyed: ")
                if confirm.upper() != 'SACRIFICE':
                    print("   ❌ Sacrifice not confirmed. Please choose strategy again.")
                    continue
            break
    ...
    if not source_apps:
        print("   ❌ No apps found in source site")
        return
    ...
    while True:
        ...
        if result == 'BACK':
            ...
            print(f"   ✅ Selected source app: {source_app}")
            break
    ...
    while True:
        ...
        if result == 'BACK':
            ...
            if target_app == source_app:
                print("   ❌ Target app cannot be same as source. Please choose a different app.")
                continue
                
            print(f"   ✅ Selected target app: {target_app}")
            break
    ...
    if source_modules and len(source_modules) > 0:
        ...
        while True:
            ...
            options = validator.format_menu_options(
                ...
                allow_exit=True, 
                allow_all=True,
                default_all=True  # ✅ DEFAULT ALL ENABLED
            )
            for option in options:
                print(option)
            ...
            if user_input == validator.BACK_KEY:
                ...
                print("   ✅ Will migrate all modules (default)")
                break
            ...
            try:
                ...
                if invalid_indices:
                    print(f"   ⚠️  Skipping invalid indices: {invalid_indices}")
                ...
                if selected_modules:
                    ...
                    print("   ❌ No valid modules selected. Please try again.")
            except ValueError:
                print("   ❌ Invalid input. Please enter numbers separated by commas.")
    else:
        print("   ℹ️  No modules found in source app, will proceed with app-level migration")
    
    # STEP 7: EXECUTE MIGRATION
    print(f"\n🚀 STEP 7: EXECUTING ENTERPRISE MIGRATION")
    print(f"   Source: {source_site}::{source_app}")
    print(f"   Target: {target_site}::{target_app}")
    ...
    if strategy_choice == 1:
        ...
        print("   🔧 Sacrifice migration - Advanced feature coming soon")
        success = False
...
def show_app_modules(app_name):
    ...
    if not modules:
        print(f"   ❌ No modules found in {app_name}")
        return []
    ...
    try:
        ...
        for module in modules:
            if module.lower() == 'core':
                print(f"   🚫 SKIPPED: Core module files")
                skipped_count += 1
                continue
...
def migrate_app(action, source_site=None, target_site=None, source_app=None, target_app=None, modules=None, namespace=None, doctype=None, doc_type=None, module_filter=None):
    ...
    print(f"🚀 ENTERPRISE MIGRATION: {action}")
    ...
    if action == 'enterprise-wizard':
        ...
        if not all([source_site, target_site, source_app, target_app]):
            print("❌ Required: source_site, target_site, source_app, target_app")
            return
        ...
        if not all([source_site, target_site, source_app, target_app]):
            print("❌ Required: source_site, target_site, source_app, target_app")
            return
        print("🔍 Cross-Site Analysis Mode")
    ...
    # ========== MIGRATION COMMANDS ==========
        ...
        if not source_app or not target_app:
            print("❌ Source and target app required")
            return
        print("🔍 Comprehensive analysis - feature coming soon")
